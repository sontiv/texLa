...
%lshort CH1.4
\documentclass[a4paper,11pt]{book} %book,article,report,letter,slide book(even,odd)...cool convenience
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{graphicx}
\usepackage{verbatim}
...
\begin{comment} 
                %from internet
                %from classes.pdf in D:\MiKTeX 2.9\doc\latex\base
...
\begin{comment} % in Ali-ben debian OR use git-bash here(NOT use winEdt's cli)
12711  xdvi lshort-3_2_0\(CH1.5\).dvi
12712  dvips -Pcmz lshort-3_2_0\(CH1.5\).dvi -o foo.ps
12713  echo $?
\end{comment}

\paragraph{lshort 1.6}bu4ju2 %???拼音注音
% 文档类
\\f(loat)l(eft)eqn
\\l(eft)eqno(rder)
% \usepackage{graphicx}
...
sty -> D:\MiKTeX 2.9\tex\latex
\\dtx -> Desktop\KIKI贵鬼\Math\TUG\latex-graphics.tds\source\latex\graphics
